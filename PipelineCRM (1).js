...
${buildPipelineContext()}`;
  ...
  if (!open) return (
    <button onClick={() => setOpen(true)} style={{
      ...
      boxShadow: "0 4px 24px rgba(15,23,42,.4)",
      fontFamily: "'DM Sans',sans-serif",
    }}>
      <span style={{ fontSize: 18 }}>🎯</span>
      <div style={{ textAlign: "left" }}>
        <div style={{ fontSize: 13, fontWeight: 700 }}>Sales Master</div>
        <div style={{ fontSize: 10, opacity: 0.7 }}>{deals.filter(d => !["Closed Won", "Closed Lost"].includes(d.stage)).length} deals live</div>
      </div>
    </button>
  );
  ...
  return (
    <div style={{
      ...
      {/* Header */}
      <div style={{ padding: "14px 18px", background: "linear-gradient(135deg,#0f172a,#1e3a8a)", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <div style={{ fontSize: 14, fontWeight: 700, color: "#fff" }}>Sales Master</div>
            <div style={{ fontSize: 10, color: "rgba(255,255,255,.65)" }}>
              Senior Sales Consultant · {deals.filter(d => !["Closed Won", "Closed Lost"].includes(d.stage)).length} active deals · ₹{deals.filter(d => !["Closed Won", "Closed Lost"].includes(d.stage)).reduce((s, d) => s + pv(d.value), 0).toFixed(1)}L pipeline
            </div>
          </div>
        </div>
        <div style={{ display: "flex", gap: 7 }}>
          {msgs.length > 0 && <button onClick={() => { setMsgs([]); setShowWelcome(true); }} style={{ background: "rgba(255,255,255,.12)", border: "none", borderRadius: 6, padding: "4px 10px", color: "#fff", fontSize: 11, cursor: "pointer", fontFamily: "inherit" }}>Clear</button>}
          <button onClick={() => setOpen(false)} style={{ background: "rgba(255,255,255,.15)", border: "none", borderRadius: 7, width: 27, height: 27, color: "#fff", fontSize: 15, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>×</button>
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "14px 16px 8px", display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        {showWelcome && msgs.length === 0 && (
          <div>
            <div style={{ background: "#f8fafc", borderRadius: 12, padding: "14px 16px", marginBottom: 12, fontSize: 13, color: C.text, lineHeight: 1.6 }}>
              <strong>I know your full pipeline.</strong> Ask me anything — qualify a new lead, draft a follow-up, get a forecast, or just think through a deal together.
            </div>
            <div style={{ fontSize: 11, fontWeight: 600, color: C.textDim, marginBottom: 8, letterSpacing: 0.5 }}>QUICK START</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {QUICK.map((q, i) => (
                <button key={i} onClick={() => send(q)} style={{
                  padding: "9px 14px", background: C.white, border: `1px solid ${C.border}`,
                  borderRadius: 9, color: C.textMid, fontSize: 12, textAlign: "left",
        ...
        {msgs.map((m, i) => (
          <div key={i} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start", alignItems: "flex-end", gap: 8 }}>
            {m.role === "assistant" && (
              <div style={{ width: 26, height: 26, borderRadius: "50%", background: "linear-gradient(135deg,#0f172a,#1e3a8a)", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, color: "#fff", marginBottom: 2 }}>🎯</div>
            )}
            <div style={{
              maxWidth: "82%", padding: "10px 14px",
              borderRadius: m.role === "user" ? "16px 16px 4px 16px" : "4px 16px 16px 16px",
              background: m.role === "user" ? "#1e3a8a" : "#f8fafc",
        ...
        {loading && (
          <div style={{ display: "flex", alignItems: "flex-end", gap: 8 }}>
            <div style={{ width: 26, height: 26, borderRadius: "50%", background: "linear-gradient(135deg,#0f172a,#1e3a8a)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, color: "#fff" }}>🎯</div>
            <div style={{ padding: "10px 14px", background: "#f8fafc", borderRadius: "4px 16px 16px 16px", border: `1px solid ${C.border}`, display: "flex", gap: 5 }}>
              {[0, 1, 2].map(i => <div key={i} style={{ width: 7, height: 7, borderRadius: "50%", background: "#1e3a8a", animation: `blink 1.2s ${i * .2}s infinite` }} />)}
            </div>
          </div>
        )}
        <div ref={bottomRef} />
      ...
      <div style={{ padding: "10px 14px 14px", borderTop: `1px solid ${C.border}`, flexShrink: 0 }}>
        {recording && (
          <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "5px 10px", background: C.redLight, borderRadius: 7, marginBottom: 7, fontSize: 11, color: C.red }}>
            <div style={{ width: 7, height: 7, borderRadius: "50%", background: C.red, animation: "blink .8s infinite" }} /> Listening...
          </div>
        )}
        <div style={{ display: "flex", gap: 7, alignItems: "flex-end" }}>
          <textarea value={input} onChange={e => setInput(e.target.value)}
            onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); send(); } }}
            placeholder="Tell me about a new lead, ask about a deal, or just think out loud..."
            rows={2} style={{ flex: 1, background: "#f8fafc", border: `1px solid ${C.border}`, borderRadius: 10, padding: "9px 12px", fontSize: 13, fontFamily: "inherit", outline: "none", resize: "none", color: C.text, lineHeight: 1.5 }} />
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            <button onClick={recording ? () => { recRef.current?.stop(); setRecording(false); } : startVoice}
              style={{ width: 36, height: 36, borderRadius: "50%", background: recording ? C.redLight : "#eff6ff", border: `1.5px solid ${recording ? C.red : C.accent}`, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, color: recording ? C.red : C.accent }}>
              {recording ? "⏹" : "🎙"}
            </button>
            <button onClick={() => send()} disabled={!input.trim() || loading}
              style={{ width: 36, height: 36, borderRadius: "50%", background: input.trim() && !loading ? "#1e3a8a" : "#e2e8f0", border: "none", cursor: input.trim() && !loading ? "pointer" : "default", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, color: input.trim() && !loading ? "#fff" : C.textFaint }}>
              →
            </button>
          </div>
        </div>
